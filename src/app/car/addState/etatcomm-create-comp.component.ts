...
@Component({
  ...
  templateUrl: './etatcomm-create-comp.component.html',
  styleUrls: ['./state-create-comp.component.scss']
})

export class EtatCommCompComponent implements OnInit {
	 public form =null;
...
isActive:boolean=false;
    ...
    doSaveLocalisation() {
	   ...
	   this.localis=new Localisation(formData.idEtatComercial,formData.libelle,formData.numorder);
	   this.localis.idEtatComercial=formData.idEtatComercial
	   header.append('Content-Type', 'application/json');
	  
   
   
	this.carSer.saveEtatComm(this.localis).subscribe(
        (data) => {
  ...
  loadLocal(idLocal){
	   this.carSer.geEtatCommDetail(idLocal ).subscribe(data => {	
	  this.form.patchValue({idEtatComercial: data['idLocalisation'].toString() });
	  this.form.patchValue({newMarque: false});
