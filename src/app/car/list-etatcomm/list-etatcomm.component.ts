...
@Component({
  ...
  templateUrl: './list-etatcomm.component.html',
  styleUrls: ['./list-marque.component.scss']
})
export class ListEtatCommComponent implements OnInit {
rows= [];
AllAccount= [];
AllAccountPr= [];
  count = 0;
  ...
  fetch(cb) {
		...
		this.carSer.getAllEtatComm().subscribe(data => {		 
  ...
  localDetail(value){
	 this.router.navigate(["car/etatCom-add/"+value]);
