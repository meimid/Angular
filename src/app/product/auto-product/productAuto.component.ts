...
@Component({
  selector: 'prd-auto',
  templateUrl: './productAuto.component.html',
  styleUrls: ['./productAuto.component.scss']
})
export class ProductAutoComponent implements OnChanges, OnInit {
  ...
  constructor(private fb: FormBuilder, translate: TranslateService, private prdSer: ProductService, private router: Router, private http: AuthHttp, ) {
    ...
    this.prdSer.getAllProudct().subscribe(data => { this.AllPersonneFliter = data; });
  }
  ...
  ngOnInit() {
    ...
    this.filteredOptions = this.prdSer.getAllProudct();
