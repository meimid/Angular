...
export class SigninChangUserPassdComponent implements OnInit {
 model: any = {};
    loading = false;
    error = '';
  public form: FormGroup;
   ...
   //,private accountSer,
    userLogin:string='';
	private sub: any;
  constructor(private fb: FormBuilder,private http: AuthHttp, private router: Router,private route: ActivatedRoute,private translate: TranslateService,private snackBar: MatSnackBar, private accountSer:AccountService) 
  {
  ...
  onSubmit() {
	...
	formData['currentPassword']=md5(formData.currentPassword);
        this.accountSer.saveNewPasswordADM(formData)
            .subscribe(result => {
